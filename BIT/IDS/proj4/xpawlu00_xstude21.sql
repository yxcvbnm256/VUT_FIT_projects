...
CREATE OR REPLACE PROCEDURE tableDrop(name IN VARCHAR2)
AS
BEGIN
...
CREATE OR REPLACE PROCEDURE sequenceDrop(name IN VARCHAR2)
AS
BEGIN
...
CREATE OR REPLACE PROCEDURE indexDrop(name IN VARCHAR2)
AS
BEGIN
...
EXECUTE tableDrop('Druh_caje');
EXECUTE tableDrop('Zeme');
EXECUTE tableDrop('Oblast');
EXECUTE tableDrop('Caj');
EXECUTE tableDrop('Kvalita');
EXECUTE tableDrop('Hodnoceni');
EXECUTE tableDrop('Dodavatel');
EXECUTE tableDrop('Varka');
EXECUTE tableDrop('Objednavka');
EXECUTE tableDrop('Mnozstvi');
EXECUTE tableDrop('Odberatel');
EXECUTE tableDrop('Firma');
EXECUTE tableDrop('Fyzicka_osoba');

EXECUTE sequenceDrop('Varka_seq'); 
...
-----------------------------------------PROCEDURE - nejdrazsi caj-----------------------------------------

CREATE OR REPLACE PROCEDURE Nejdrazsi (idVarka OUT NUMBER, maxCena OUT Varka.cena%TYPE) IS
  CURSOR        cur IS
                SELECT Varka.id_varka, Varka.cena FROM Varka;
  tatoVarka     Varka.id_varka%TYPE; 
  tatoCena      Varka.cena%TYPE; 
BEGIN 
  maxCena := 0; 
  OPEN cur; 
  LOOP 
     FETCH cur INTO tatoVarka, tatoCena; 
     EXIT WHEN cur%NOTFOUND;     
     IF tatoCena > maxCena THEN 
        maxCena := tatoCena; 
        idVarka := tatoVarka; 
      END IF; 
  END LOOP; 
...
SET SERVEROUTPUT ON;
DECLARE idVarka NUMBER;
        maxCena NUMBER;  
BEGIN
  NEJDRAZSI(idVarka, maxCena);
  DBMS_OUTPUT.put_line('OutParam1: ' || idVarka);
  DBMS_OUTPUT.put_line('OutParam2: ' || maxCena);
END;
/
...
EXECUTE indexDrop('indexcena');

EXPLAIN PLAN
  SET statement_id = 'opt1'
FOR
...
from table(dbms_xplan.display('plan_table','opt1','all'));


CREATE INDEX indexcena
  ON Varka (cena);
...
-----------------------------------------TRIGGER - kontrola ico-----------------------------------------

CREATE OR REPLACE FUNCTION getDigit(input IN NUMBER, pozice IN NUMBER)
RETURN NUMBER
AS
BEGIN
  RETURN MOD(TRUNC(input/POWER(10,8-pozice)),10);
END;
/
CREATE OR REPLACE FUNCTION computeLastDigit(input IN NUMBER)
RETURN NUMBER
AS
BEGIN
  RETURN MOD(11 - MOD(getDigit(input,1)*8 + getDigit(input,2)*7 + getDigit(input,3)*6 + getDigit(input,4)*5 + getDigit(input,5)*4 + getDigit(input,6)*3 + getDigit(input,7)*2, 11), 10);
END;
/
CREATE OR REPLACE TRIGGER tr_check_ICO
  BEFORE INSERT OR UPDATE OF ICO ON Firma
  FOR EACH ROW
BEGIN
  IF( getDigit(:new.ICO,8) <> computeLastDigit(:new.ICO) )
  THEN
    RAISE_APPLICATION_ERROR( -20001, 'Zadane neplatne ICO' );
  END IF;
END;
/
...
CREATE SEQUENCE Varka_seq
  MINVALUE 1
  INCREMENT BY 1;


CREATE OR REPLACE TRIGGER Varka_tr
BEFORE INSERT ON Varka
REFERENCING NEW AS NEW 
FOR EACH ROW
BEGIN
  SELECT Varka_seq.NEXTVAL
  INTO   :new.id_varka
  FROM   dual;
END;
/
...
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0001', '0001', TO_DATE('20151506', 'YYYYDDMM'), 'autem', '40', '1090', '0');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0001', '0001', TO_DATE('20151206', 'YYYYDDMM'), 'lodi','50', '2090', '0');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0004', '0002', TO_DATE('20151306', 'YYYYDDMM'), 'autem','40', '1390', '30');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0005', '0002', TO_DATE('20142306', 'YYYYDDMM'), 'letadlem','60', '1590', '20');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0007', '0004', TO_DATE('20151503', 'YYYYDDMM'), 'letadlem','40', '1390', '0');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0008', '0001', TO_DATE('20151304', 'YYYYDDMM'), 'lodi','120', '1190', '20');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0009', '0002', TO_DATE('20151101', 'YYYYDDMM'), 'autem','80', '980', '0');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0010', '0004', TO_DATE('20150303', 'YYYYDDMM'), 'letadlem','40', '1490', '0');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0003', '0001', TO_DATE('20150404', 'YYYYDDMM'), 'lodi','290', '2090', '0');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0002', '0003', TO_DATE('20152004', 'YYYYDDMM'), 'lodi','40', '3090', '10');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0005', '0002', TO_DATE('20153006', 'YYYYDDMM'), 'letadlem','410', '1590', '0');
INSERT INTO  Varka(id_caj, id_dodavatel, datum_expedice, druh_prepravy, mnozstvi, cena, sleva)
VALUES('0006', '0001', TO_DATE('20151706', 'YYYYDDMM'), 'autem','140', '1990', '0');
...
INSERT INTO Firma
VALUES('0001', '0001', '14637448', 'Příšerky s.r.o');
INSERT INTO Firma
VALUES('0002', '0003', '14637448', 'Příšerky s.r.o');
INSERT INTO Firma
VALUES('0003', '0005', '14637448', 'Příšerky s.r.o');
...
BEGIN
  FOR c IN cur2
  LOOP
    cmd := 'GRANT SELECT ON '||c.table_name||' TO XSTUDE21';
    EXECUTE IMMEDIATE cmd;
  END LOOP;
END;
/

------------materializovany pohled - spousti se od cloveka, kteremu byly prideleny prava---------
