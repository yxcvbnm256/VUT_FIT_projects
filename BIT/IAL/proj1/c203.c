...
*/
	if (q != NULL) 
	{
		...
		{
			q->arr[i] = '*';
			/* Cyklus for naplni cele pole hvezdickama */
		}
...
*/
	...
	/* Pokud se index f a b rovnaji, je fronta prazdna */
}
...
*/
	...
	/* Pokud je index b o jedno mensi, nez f, je fronta plna */
}
...
*/
	...
	{
		*c = q->arr[q->f_index];
		/* Pevni prvek = prvek pole na indexu f */
	}
...
*/
	if (queueEmpty(q) == 0)
	{
		...
		/* Pokud neni fronta prazdna, prvek se vymaze, neboli se index f posune o jedna. */
	}
...
*/
	if (queueEmpty(q) == 0)
	{
		*c = q->arr[q->f_index];
		q->f_index = nextIndex(q->f_index);	
		/* Kombinace funkci queueRemove a queueFront */	
	}
...
*/
	if (queueFull(q) == 0)
	{
		...
		/* Pokud fronta neni plna, do pole na index b se nahraje novy znak. Pote se index b posune o jedna. */
	}
